...
If you have built an agentic pipeline against an API and are looking at the bill, the inside-out flip is worth a weekend — once the patterns are in hand. Today's work was the opposite of automatic. We hunted a silent guard bug that had been blocking every heredoc command. We walked Soviet Supermarket back a step when Claude Code's safety heuristic flagged long heredoc content. There were a handful of other format and structural rewrites along the way that aren't worth itemizing. The patterns are leverage, not autopilot. The CLI is a few hundred lines, and most of the existing project comes along unchanged.
